    ...
    <header className="bg-background sticky top-0 z-40 w-full border-b text-gray-800">
      <div className="container flex h-16 items-center space-x-4 sm:justify-between sm:space-x-0">
        ...
        <div className="flex flex-1 items-center justify-end space-x-4">
          <button
            className={`text-2xl p-2 focus:outline-none border-none rounded transition-all duration-200 ${isOpen ? "bg-gray-800 text-white" : "bg-transparent text-gray-800"} hover:bg-[#E4003A]/10 hover:text-[#E4003A] focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 md:hidden`}
            onClick={toggleMenu}
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-95 z-20 overflow-y-auto p-4">
          <button
            className="absolute top-4 right-4 text-3xl text-white focus:outline-none hover:text-red-500"
            onClick={toggleMenu}
          ...
          <div className="mt-8 flex justify-center space-x-4  invisible lg:visible">
            <Link href={siteConfig.links.github} target="_blank" rel="noreferrer">
              <div className="h-10 w-10 flex items-center justify-center bg-gray-800 text-white rounded-full hover:bg-gray-700 hover:text-red-500 focus:ring-2 focus:ring-gray-500">
                <Icons.gitHub className="h-5 w-5" />
                <span className="sr-only">GitHub</span>
